...
class D_Monster:public Monster{
	...
	public:
		...
		void Skill(){
			if(((m.MAP_2DARRAY_X_YVal(x,y) >= F_NUMBER) && (m.MAP_2DARRAY_X_YVal(x,y) <= T_NUMBER)) && (skill_count > 0)){
	...
	public:
		...
		void GenerateMonster(){
			...
			if((rate >= 0) && (rate <= 2)){
				...
				Insert_C_monster();
				gotoxy(30,27);
				cout<<"generate C_monster"<<endl;
				return;
			}
			else if(rate == 8){
				Insert_J_monster();
				gotoxy(30,27);
				cout<<"generate J_monster"<<endl;
				return;
			}
			else if(rate == 9){
				Insert_W_monster();
				gotoxy(30,27);
				cout<<"generate W_monster"<<endl;
				return;
			}
		}
		void AddScore(int a){
		...
		void Check_buffEat(Object *object){
			...
			if(object->get_icon() == 'D'){
				D_Monster *d;
				if((m.MAP_2DARRAY_X_YVal(object->ReturnX(),object->ReturnY()) >= 3) && (m.MAP_2DARRAY_X_YVal(object->ReturnX(),object->ReturnY()) <= 6) && (d->SkillVal() > 0)){
					B->Delete_Buff(object->ReturnX(),object->ReturnY());
				}
			}
			else{
		...
		void Delete_Object(Object *delete_Object){
  			...
  			J_Monster *J;
  			D_Monster *d;
			if(delete_Object->get_icon() == 'J'){
                J->set_skill();
            }
            if(delete_Object->get_icon() == 'D'){
            	d->ResetSkillVal();
			}
			do{
		...
		void Jeremy(){
			...
			if(J_live() && (5<=duration && duration<=6) && (J->return_skill()<3)){
				J_start_time = clock();
				J->use_skill();
				Object *p = first;
				do{
